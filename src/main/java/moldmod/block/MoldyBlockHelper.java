...
public class MoldyBlockHelper {
    ...
    public static boolean hasRandomTicks(BlockState state) {
        return !state.get(MoldyOakLogBlock.WAXED);
    }

    public static double calculateR(ServerWorld world, BlockPos pos, boolean isWaxed, BlockState stateToCheck) {
        ...
        if (isWaxed) {
            ...
            if (stateToCheck.isOf(ModBlocks.MOLDY_STRIPPED_OAK_LOG)) Smat = 1.4;
            else if (stateToCheck.isOf(Blocks.STRIPPED_OAK_LOG)) Smat = 1.4;
            else if (stateToCheck.isOf(ModBlocks.MOLDY_OAK_PLANKS)) Smat = 0.8;
    ...
    public static void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random, net.minecraft.block.Block block) {
        ...
        if (state.get(MoldyOakLogBlock.WAXED)) return;

        double R = calculateR(world, pos, state.get(MoldyOakLogBlock.WAXED), state) * config.globalMoldRiskMultiplier;
        ...
        if (R > 0.65) {
            int currentStage = state.get(MoldyOakLogBlock.STAGE);
            if (currentStage < 3) {
                world.setBlockState(pos, state.with(MoldyOakLogBlock.STAGE, currentStage + 1));
            }
        }
        ...
        if (neighborState.isOf(block) && !neighborState.get(MoldyOakLogBlock.WAXED) && neighborState.get(MoldyOakLogBlock.STAGE) < 3) {
            if (calculateR(world, neighborPos, false, neighborState) * config.globalMoldRiskMultiplier > 0.65) {
                world.setBlockState(neighborPos, neighborState.with(MoldyOakLogBlock.STAGE, neighborState.get(MoldyOakLogBlock.STAGE) + 1));
            }
        }
    }
    ...
    public static ItemActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, net.minecraft.block.Block strippedStateBlock) {
        if (stack.getItem() instanceof net.minecraft.item.AxeItem) {
            if (state.get(MoldyOakLogBlock.WAXED)) {
                world.setBlockState(pos, state.with(MoldyOakLogBlock.WAXED, false));
            ...
            } else if (state.get(MoldyOakLogBlock.STAGE) == 1) {
                world.setBlockState(pos, state.with(MoldyOakLogBlock.STAGE, 0));
                ...
                stack.damage(1, player, PlayerEntity.getSlotForHand(hand));
                return ItemActionResult.SUCCESS;
            } else if (state.get(MoldyOakLogBlock.STAGE) == 0 && strippedStateBlock != null) {
            ...
            if (!state.get(MoldyOakLogBlock.WAXED)) {
                world.setBlockState(pos, state.with(MoldyOakLogBlock.WAXED, true));
