...
public final class MoistureDetectorBlock extends WallMountedBlock {
    ...
    public void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        if (state.get(MOISTURE_STAGE) != stage) {
            BlockState newState = state.with(MOISTURE_STAGE, stage);
    ...
    public boolean emitsRedstonePower(BlockState state) {
        return state.get(MOISTURE_STAGE) > 0;
    }

    @Override
    public int getWeakRedstonePower(BlockState state, BlockView world, BlockPos pos, Direction direction) {
        int stage = state.get(MOISTURE_STAGE);
        ModConfig config = getConfig();
        int mult = (config != null && config.moistureDetector != null)
                ? config.moistureDetector.redstone_level_multiplier
                : 5;
        return MathHelper.clamp(stage * mult, 0, 15);
    }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        if (!world.isClient && world instanceof ServerWorld serverWorld) {
            ...
            world.playSound(null, pos, SoundEvents.BLOCK_COPPER_BULB_TURN_ON, SoundCategory.BLOCKS, 0.8f, 1.1f);
        }
