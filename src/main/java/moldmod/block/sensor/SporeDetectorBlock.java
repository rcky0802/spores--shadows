...
public final class SporeDetectorBlock extends WallMountedBlock {
    ...
    public void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        if (state.get(TOXICITY_LEVEL) != toxLevel) {
            BlockState newState = state.with(TOXICITY_LEVEL, toxLevel);
    ...
    public boolean emitsRedstonePower(BlockState state) {
        return state.get(TOXICITY_LEVEL) > 0;
    }

    @Override
    public int getWeakRedstonePower(BlockState state, BlockView world, BlockPos pos, Direction direction) {
        int toxLevel = state.get(TOXICITY_LEVEL);
        ModConfig config = getConfig();
        int mult = (config != null && config.sporeDetector != null)
                ? config.sporeDetector.redstone_level_multiplier
                : 5;
        return MathHelper.clamp(toxLevel * mult, 0, 15);
    }
    ...
    private void updateNeighbors(BlockState state, World world, BlockPos pos) {
        ...
        world.updateComparators(pos.offset(getDirection(state).getOpposite()), this);
    }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        if (!world.isClient && world instanceof ServerWorld serverWorld) {
            ...
            world.playSound(null, pos, SoundEvents.BLOCK_COPPER_BULB_TURN_ON, SoundCategory.BLOCKS, 0.8f, 1.3f);
        }
