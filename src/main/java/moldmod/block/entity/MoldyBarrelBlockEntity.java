...
import moldmod.block.MoldyBarrelBlock;
...
public class MoldyBarrelBlockEntity extends BarrelBlockEntity {

    private int moldStage = 0;
    private boolean moldWaxed = false;

    public MoldyBarrelBlockEntity(BlockPos pos, BlockState state) {
        super(pos, state);
        if (state != null) {
    ...
    public BlockEntityType<?> getType() {
        return ModBlockEntities.MOLDY_BARREL != null ? ModBlockEntities.MOLDY_BARREL : super.getType();
    }

    @Override
    public boolean supports(BlockState state) {
        return state.getBlock() instanceof MoldyBarrelBlock || super.supports(state);
    }
