...
public class MoldyOakLogBlock extends PillarBlock {
    ...
    public MoldyOakLogBlock(Settings settings, Block strippedBlock) {
        super(settings);
        this.strippedBlock = strippedBlock;
        this.setDefaultState(this.getDefaultState().with(AXIS, net.minecraft.util.math.Direction.Axis.Y).with(STAGE, 0).with(WAXED, false));
    }

    @Override
    protected void appendProperties(StateManager.Builder<Block, BlockState> builder) {
        super.appendProperties(builder);
        builder.add(STAGE, WAXED);
    }
