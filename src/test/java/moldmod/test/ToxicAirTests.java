...
public class ToxicAirTests {
    ...
    public void testCleanAirThresholds(TestContext context) {
        ...
        ToxicAirEvent.MiasmaResult clean = new ToxicAirEvent.MiasmaResult(0.0, 0.0, false, 10, java.util.Collections.emptySet());
        if (clean.level != ToxicAirEvent.AirToxicityLevel.CLEAN) {
            context.throwPositionedException("Miasma a 0 deve essere CLEAN, trovato: " + clean.level, new BlockPos(0, 1, 0));
        }

        // Open Air -> sempre CLEAN
        ToxicAirEvent.MiasmaResult openAir = new ToxicAirEvent.MiasmaResult(50.0, 0.0, true, 10, java.util.Collections.emptySet());
        if (openAir.level != ToxicAirEvent.AirToxicityLevel.CLEAN) {
            context.throwPositionedException("Open Air deve essere sempre CLEAN, trovato: " + openAir.level, new BlockPos(0, 1, 0));
        }
        if (openAir.netMiasma != 0.0) {
            context.throwPositionedException("Open Air deve avere netMiasma = 0.0, trovato: " + openAir.netMiasma, new BlockPos(0, 1, 0));
        }
        ...
        // Volume >= MAX_AIR_VOLUME -> UNCONFINED_CAVERN (miasma diluted in massive volume -> CLEAN)
        ToxicAirEvent.MiasmaResult hugeRoom = new ToxicAirEvent.MiasmaResult(5.0, 0.0, false, 800, java.util.Collections.emptySet());
        if (hugeRoom.level != ToxicAirEvent.AirToxicityLevel.CLEAN) {
            context.throwPositionedException("Stanza >= 800 blocchi con bassa muffa deve essere considerata CLEAN per diluizione, trovato: " + hugeRoom.level, new BlockPos(0, 1, 0));
        }
        if (hugeRoom.ventilationType != ToxicAirEvent.RoomVentilationType.UNCONFINED_CAVERN) {
            context.throwPositionedException("Stanza >= 800 blocchi deve risultare UNCONFINED_CAVERN, trovato: " + hugeRoom.ventilationType, new BlockPos(0, 1, 0));
        }
    ...
    public void testMiasmaDensityThresholds(TestContext context) {
        // Stanza piccola (volume 5) con 5 tronchi stadio 3 marci (toxicScore = 5 * 3 * 1.5 = 22.5)
        // netMiasma = 22.5, densità = 22.5 / 5 = 4.5 -> LETHAL_POISON
        ToxicAirEvent.MiasmaResult smallDenseRoom = new ToxicAirEvent.MiasmaResult(22.5, 0.0, false, 5, java.util.Collections.emptySet());
        if (smallDenseRoom.level != ToxicAirEvent.AirToxicityLevel.LETHAL_POISON) {
            context.throwPositionedException("Stanza 5 blocchi con 22.5 tossicità deve essere LETHAL_POISON, trovato: " + smallDenseRoom.level, new BlockPos(0, 1, 0));
        }

        // Stanza media (volume 20) con 2 tronchi stadio 2 (toxicScore = 2 * 2 * 1.5 = 6.0)
        // netMiasma = 6.0, densità = 6.0 / 20 = 0.30 -> MODERATE_HUNGER
        ToxicAirEvent.MiasmaResult mediumRoom = new ToxicAirEvent.MiasmaResult(6.0, 0.0, false, 20, java.util.Collections.emptySet());
        if (mediumRoom.level != ToxicAirEvent.AirToxicityLevel.MODERATE_HUNGER) {
            context.throwPositionedException("Stanza media con 6.0 tossicità deve essere MODERATE_HUNGER, trovato: " + mediumRoom.level, new BlockPos(0, 1, 0));
        }

        // Stanza grande (volume 100) con 3 tronchi stadio 1 (toxicScore = 3 * 1 * 1.5 = 4.5)
        // netMiasma = 4.5, densità = 4.5 / 100 = 0.045 -> WARNING
        ToxicAirEvent.MiasmaResult largeRoom = new ToxicAirEvent.MiasmaResult(4.5, 0.0, false, 100, java.util.Collections.emptySet());
        if (largeRoom.level != ToxicAirEvent.AirToxicityLevel.WARNING) {
            context.throwPositionedException("Stanza grande con 4.5 tossicità deve essere WARNING, trovato: " + largeRoom.level, new BlockPos(0, 1, 0));
        }
    ...
    public void testDensityScalingWithVolume(TestContext context) {
        ...
        ToxicAirEvent.MiasmaResult small = new ToxicAirEvent.MiasmaResult(12.0, 0.0, false, 6, java.util.Collections.emptySet());
        ToxicAirEvent.MiasmaResult large = new ToxicAirEvent.MiasmaResult(12.0, 0.0, false, 60, java.util.Collections.emptySet());

        if (small.density <= large.density) {
            context.throwPositionedException("La densità nella stanza piccola (" + small.density + ") deve essere maggiore della grande (" + large.density + ")", new BlockPos(0, 1, 0));
        }
        if (small.exposureIndex <= large.exposureIndex) {
            context.throwPositionedException("L'indice di esposizione nella stanza piccola (" + small.exposureIndex + ") deve essere maggiore della grande (" + large.exposureIndex + ")", new BlockPos(0, 1, 0));
        }
    ...
    public void testSealedRoomToxicity(TestContext context) {
        // Costruiamo una stanza 3x3x3 in pietra chiusa (1 blocco d'aria al centro: (2, 2, 2))
    ...
    public void testOpenAirThroughSlabGaps(TestContext context) {
        ...
        // Parete con bottom slab che affaccia l'apertura laterale verso il centro (1, 2, 2) ed esterno a Ovest (0, 2, 2) con cielo aperto sopra (0, 3, 2)
        ...
        if (result.ventilationType != ToxicAirEvent.RoomVentilationType.VENTILATED) {
            context.throwPositionedException("La mezza lastra verso l'esterno deve conferire VENTILATED, trovato: " + result.ventilationType, center);
        }
        if (result.ventilationScore <= 0.0) {
            context.throwPositionedException("La mezza lastra verso l'esterno deve conferire un bonus di ventilazione > 0!", center);
        }
    ...
    public void testStairsDirectionalAirPassage(TestContext context) {
        ...
        // 1. Scala con retro solido rivolto verso l'interno (FACING = WEST -> retro a EST verso la stanza) -> BLOCCA (HERMETIC_SEALED)
        ...
        ToxicAirEvent.MiasmaResult resultBackToRoom = ToxicAirEvent.calculateMiasma(context.getWorld(), context.getAbsolutePos(center));
        if (resultBackToRoom.ventilationType != ToxicAirEvent.RoomVentilationType.HERMETIC_SEALED) {
            context.throwPositionedException("Scala con retro solido verso l'interno deve risultare HERMETIC_SEALED, trovato: " + resultBackToRoom.ventilationType, center);
        }

        // 2. Scala con gradino verso la stanza e retro solido verso l'esterno (FACING = EAST -> retro a OVEST verso l'esterno) -> BLOCCA (HERMETIC_SEALED)
        ...
        ToxicAirEvent.MiasmaResult resultBackToOutside = ToxicAirEvent.calculateMiasma(context.getWorld(), context.getAbsolutePos(center));
        if (resultBackToOutside.ventilationType != ToxicAirEvent.RoomVentilationType.HERMETIC_SEALED) {
            context.throwPositionedException("Scala con gradino verso la stanza e retro solido verso l'esterno deve risultare HERMETIC_SEALED, trovato: " + resultBackToOutside.ventilationType, center);
        }

        // 3. Scala orientata lateralmente (FACING = SOUTH) -> il profilo aperto permette il passaggio trasversale verso l'esterno a OVEST -> VENTILATED
        ...
        ToxicAirEvent.MiasmaResult resultSideways = ToxicAirEvent.calculateMiasma(context.getWorld(), context.getAbsolutePos(center));
        if (resultSideways.ventilationType != ToxicAirEvent.RoomVentilationType.VENTILATED) {
            context.throwPositionedException("Scala con profilo aperto trasversale deve risultare VENTILATED, trovato: " + resultSideways.ventilationType, center);
        }
        if (resultSideways.ventilationScore <= 0.0) {
            context.throwPositionedException("Scala con profilo aperto trasversale deve conferire un bonus di ventilazione > 0!", center);
        }
    ...
    public void testDoorOpenVsClosedAirFlow(TestContext context) {
        ...
        if (resultClosed.volume != 1) {
            context.throwPositionedException("A porta chiusa il volume della stanza 1 deve essere 1, trovato: " + resultClosed.volume, new BlockPos(1, 2, 2));
        }
        ...
        if (resultOpen.volume <= 1) {
            context.throwPositionedException("A porta aperta il volume d'aria deve espandersi nella stanza adiacente, trovato: " + resultOpen.volume, new BlockPos(1, 2, 2));
        }
    ...
    public void testDoorBothHalvesAirtightVsOpen(TestContext context) {
        // Due stanze separate da una parete divisoria con una porta da 2 blocchi di altezza
        for (int x = 0; x <= 4; x++) {
            for (int y = 1; y <= 4; y++) {
                for (int z = 1; z <= 3; z++) {
                    context.setBlockState(new BlockPos(x, y, z), Blocks.STONE.getDefaultState());
        ...
        // 1. A porta CHIUSA (entrambi i blocchi): camera stagna, volume = 2 blocchi della sola stanza 1
        ToxicAirEvent.MiasmaResult resultClosed = ToxicAirEvent.calculateMiasma(context.getWorld(), context.getAbsolutePos(new BlockPos(1, 2, 2)));
        if (resultClosed.volume != 2) {
            context.throwPositionedException("A porta chiusa (entrambi i blocchi) il volume deve essere esattamente 2, trovato: " + resultClosed.volume, new BlockPos(1, 2, 2));
        }

        // 2. A porta APERTA (entrambi i blocchi): l'aria fluisce liberamente attraverso entrambi i blocchi della porta (volume >= 4)
        ...
        if (resultOpen.volume < 4) {
            context.throwPositionedException("A porta aperta l'aria deve fluire attraverso entrambi i blocchi della porta (volume >= 4), trovato: " + resultOpen.volume, new BlockPos(1, 2, 2));
        }
    ...
    public void testDoorPerimeterClosedIsAirtight(TestContext context) {
        ...
        ToxicAirEvent.MiasmaResult resultSpruce = ToxicAirEvent.calculateMiasma(context.getWorld(), context.getAbsolutePos(center));
        if (resultSpruce.ventilationType != ToxicAirEvent.RoomVentilationType.HERMETIC_SEALED) {
            context.throwPositionedException("La porta in abete chiusa deve risultare HERMETIC_SEALED, trovato: " + resultSpruce.ventilationType, center);
        }
    ...
    public void testTrapdoorPerimeterClosedIsAirtight(TestContext context) {
        ...
        // Botola in Abete (Spruce) Chiusa su parete (OPEN = true, piastra verticale): fa camera stagna (HERMETIC_SEALED)
        ...
        ToxicAirEvent.MiasmaResult resultSpruce = ToxicAirEvent.calculateMiasma(context.getWorld(), context.getAbsolutePos(center));
        if (resultSpruce.ventilationType != ToxicAirEvent.RoomVentilationType.HERMETIC_SEALED) {
            context.throwPositionedException("La botola in abete chiusa deve risultare HERMETIC_SEALED, trovato: " + resultSpruce.ventilationType, center);
        }
        if (resultSpruce.netMiasma <= 0.0) {
            context.throwPositionedException("In stanza ermetica il miasma deve essere calcolato!", center);
        }
    ...
    public void testLateralWallTrapdoorOpenVsClosed(TestContext context) {
        ...
        // 1. Chiusa su parete (piastra verticale a coprire il foro: in-game OPEN = true) -> Camera Stagna
        ...
        if (resultClosed.ventilationType != ToxicAirEvent.RoomVentilationType.HERMETIC_SEALED) {
            context.throwPositionedException("La botola ermetica su parete laterale chiusa deve risultare HERMETIC_SEALED!", center);
        }

        // 2. Aperta su parete (piastra orizzontale a mensola: in-game OPEN = false) -> Areazione verso l'esterno
        ...
        if (resultOpen.ventilationType != ToxicAirEvent.RoomVentilationType.CLEAN_OPEN_AIR) {
            context.throwPositionedException("La botola su parete laterale aperta deve permettere CLEAN_OPEN_AIR verso l'esterno!", center);
        }
    ...
    public void testDoorAndTrapdoorHermeticClosedVsOpen(TestContext context) {
        ...
        ToxicAirEvent.MiasmaResult resultOakDoor = ToxicAirEvent.calculateMiasma(context.getWorld(), context.getAbsolutePos(center));
        if (resultOakDoor.ventilationType != ToxicAirEvent.RoomVentilationType.HERMETIC_SEALED) {
            context.throwPositionedException("La porta in quercia chiusa deve risultare HERMETIC_SEALED, trovato: " + resultOakDoor.ventilationType, center);
        }
        ...
        ToxicAirEvent.MiasmaResult resultOakDoorOpen = ToxicAirEvent.calculateMiasma(context.getWorld(), context.getAbsolutePos(center));
        if (resultOakDoorOpen.ventilationType != ToxicAirEvent.RoomVentilationType.CLEAN_OPEN_AIR) {
            context.throwPositionedException("La porta in quercia aperta verso l'esterno deve risultare CLEAN_OPEN_AIR, trovato: " + resultOakDoorOpen.ventilationType, center);
        }

        // Ripristina soffitto sopra la porta
        context.setBlockState(doorUpper, Blocks.STONE.getDefaultState());

        // 3. Botola in Quercia su parete Chiusa (OPEN = true, piastra verticale): sempre HERMETIC_SEALED
        ...
        ToxicAirEvent.MiasmaResult resultOakTrapdoor = ToxicAirEvent.calculateMiasma(context.getWorld(), context.getAbsolutePos(center));
        if (resultOakTrapdoor.ventilationType != ToxicAirEvent.RoomVentilationType.HERMETIC_SEALED) {
            context.throwPositionedException("La botola in quercia chiusa su parete deve risultare HERMETIC_SEALED, trovato: " + resultOakTrapdoor.ventilationType, center);
        }

        // 4. Botola in Quercia su parete Aperta (OPEN = false, piastra orizzontale): sempre CLEAN_OPEN_AIR
        context.setBlockState(doorLower, oakTrapdoorClosed.with(net.minecraft.block.TrapdoorBlock.OPEN, false));
        ToxicAirEvent.MiasmaResult resultOakTrapdoorOpen = ToxicAirEvent.calculateMiasma(context.getWorld(), context.getAbsolutePos(center));
        if (resultOakTrapdoorOpen.ventilationType != ToxicAirEvent.RoomVentilationType.CLEAN_OPEN_AIR) {
            context.throwPositionedException("La botola in quercia aperta su parete deve risultare CLEAN_OPEN_AIR, trovato: " + resultOakTrapdoorOpen.ventilationType, center);
        }
    ...
    public void testCopperBlocksAndFences(TestContext context) {
        ...
        // 1. Staccionata (Fence): sempre modificatore di ventilazione verso l'esterno -> VENTILATED
        context.setBlockState(wallPos, Blocks.OAK_FENCE.getDefaultState());
        ToxicAirEvent.MiasmaResult resultFence = ToxicAirEvent.calculateMiasma(context.getWorld(), context.getAbsolutePos(center));
        if (resultFence.ventilationType != ToxicAirEvent.RoomVentilationType.VENTILATED) {
            context.throwPositionedException("La staccionata verso l'esterno deve risultare VENTILATED, trovato: " + resultFence.ventilationType, center);
        }
        if (resultFence.ventilationScore <= 0.0) {
            context.throwPositionedException("La staccionata verso l'esterno deve conferire un bonus di ventilazione > 0!", center);
        }

        // 2. Grata di rame (Copper Grate): trattata come aria (areazione totale) -> CLEAN_OPEN_AIR
        context.setBlockState(wallPos, Blocks.COPPER_GRATE.getDefaultState());
        ToxicAirEvent.MiasmaResult resultCopperGrate = ToxicAirEvent.calculateMiasma(context.getWorld(), context.getAbsolutePos(center));
        if (resultCopperGrate.ventilationType != ToxicAirEvent.RoomVentilationType.CLEAN_OPEN_AIR) {
            context.throwPositionedException("La grata di rame verso l'esterno deve risultare CLEAN_OPEN_AIR, trovato: " + resultCopperGrate.ventilationType, center);
        }
        ...
        ToxicAirEvent.MiasmaResult resultCopperDoorClosed = ToxicAirEvent.calculateMiasma(context.getWorld(), context.getAbsolutePos(center));
        if (resultCopperDoorClosed.ventilationType != ToxicAirEvent.RoomVentilationType.HERMETIC_SEALED) {
            context.throwPositionedException("La porta di rame chiusa deve risultare HERMETIC_SEALED, trovato: " + resultCopperDoorClosed.ventilationType, center);
        }
        ...
        ToxicAirEvent.MiasmaResult resultCopperDoorOpen = ToxicAirEvent.calculateMiasma(context.getWorld(), context.getAbsolutePos(center));
        if (resultCopperDoorOpen.ventilationType != ToxicAirEvent.RoomVentilationType.CLEAN_OPEN_AIR) {
            context.throwPositionedException("La porta di rame aperta verso l'esterno deve risultare CLEAN_OPEN_AIR, trovato: " + resultCopperDoorOpen.ventilationType, center);
        }

        // 5. Botola di rame su parete chiusa (OPEN = true, piastra verticale): HERMETIC_SEALED
        BlockState copperTrapdoorClosed = Blocks.COPPER_TRAPDOOR.getDefaultState()
                .with(net.minecraft.block.TrapdoorBlock.FACING, Direction.WEST)
                .with(net.minecraft.block.TrapdoorBlock.HALF, net.minecraft.block.enums.BlockHalf.BOTTOM)
                .with(net.minecraft.block.TrapdoorBlock.OPEN, true);
        context.setBlockState(wallPos, copperTrapdoorClosed);
        ToxicAirEvent.MiasmaResult resultCopperTrapdoorClosed = ToxicAirEvent.calculateMiasma(context.getWorld(), context.getAbsolutePos(center));
        if (resultCopperTrapdoorClosed.ventilationType != ToxicAirEvent.RoomVentilationType.HERMETIC_SEALED) {
            context.throwPositionedException("La botola di rame chiusa deve risultare HERMETIC_SEALED, trovato: " + resultCopperTrapdoorClosed.ventilationType, center);
        }

        // 6. Botola di rame su parete aperta (OPEN = false, piastra orizzontale): CLEAN_OPEN_AIR
        ...
        ToxicAirEvent.MiasmaResult resultCopperTrapdoorOpen = ToxicAirEvent.calculateMiasma(context.getWorld(), context.getAbsolutePos(center));
        if (resultCopperTrapdoorOpen.ventilationType != ToxicAirEvent.RoomVentilationType.CLEAN_OPEN_AIR) {
            context.throwPositionedException("La botola di rame aperta deve risultare CLEAN_OPEN_AIR, trovato: " + resultCopperTrapdoorOpen.ventilationType, center);
        }
        ...
        ToxicAirEvent.MiasmaResult resultFenceCeiling = ToxicAirEvent.calculateMiasma(context.getWorld(), context.getAbsolutePos(center));
        if (resultFenceCeiling.ventilationType != ToxicAirEvent.RoomVentilationType.VENTILATED) {
            context.throwPositionedException("La staccionata sul soffitto deve risultare VENTILATED, trovato: " + resultFenceCeiling.ventilationType, center);
        }
        if (resultFenceCeiling.ventilationScore <= 0.0) {
            context.throwPositionedException("La staccionata sul soffitto deve conferire un bonus di ventilazione > 0!", center);
        }
        ...
        ToxicAirEvent.MiasmaResult resultFenceFloor = ToxicAirEvent.calculateMiasma(context.getWorld(), context.getAbsolutePos(center));
        if (resultFenceFloor.ventilationType != ToxicAirEvent.RoomVentilationType.VENTILATED) {
            context.throwPositionedException("La staccionata sul pavimento verso l'esterno deve risultare VENTILATED, trovato: " + resultFenceFloor.ventilationType, center);
        }
        if (resultFenceFloor.ventilationScore <= 0.0) {
            context.throwPositionedException("La staccionata sul pavimento deve conferire un bonus di ventilazione > 0!", center);
        }
    ...
    public void testWallBlocksPlacement(TestContext context) {
        ...
        // 1. Muretto su parete laterale CONNESSO (1, 2, 2) -> chiude ERMETICAMENTE (HERMETIC_SEALED)
        BlockPos wallPos = new BlockPos(1, 2, 2);
        ...
        ToxicAirEvent.MiasmaResult resultWallConnected = ToxicAirEvent.calculateMiasma(context.getWorld(), context.getAbsolutePos(center));
        if (resultWallConnected.ventilationType != ToxicAirEvent.RoomVentilationType.HERMETIC_SEALED) {
            context.throwPositionedException("Il muretto connesso su parete laterale deve chiudere ermeticamente (HERMETIC_SEALED), trovato: " + resultWallConnected.ventilationType, center);
        }

        // 2. Muretto su parete laterale con SOLO 1 CONNESSIONE (1, 2, 2) -> dà bonus di ventilazione (VENTILATED)
        BlockState singleConnectedWall = Blocks.COBBLESTONE_WALL.getDefaultState()
                .with(net.minecraft.block.WallBlock.NORTH_SHAPE, net.minecraft.block.enums.WallShape.LOW);
        context.setBlockState(wallPos, singleConnectedWall);
        ToxicAirEvent.MiasmaResult resultWallSingle = ToxicAirEvent.calculateMiasma(context.getWorld(), context.getAbsolutePos(center));
        if (resultWallSingle.ventilationType != ToxicAirEvent.RoomVentilationType.VENTILATED) {
            context.throwPositionedException("Il muretto con solo 1 connessione su parete laterale deve risultare VENTILATED, trovato: " + resultWallSingle.ventilationType, center);
        }
        if (resultWallSingle.ventilationScore <= 0.0) {
            context.throwPositionedException("Il muretto con 1 connessione su parete deve conferire un bonus di ventilazione > 0!", center);
        }

        // 3. Muretto su parete laterale NON CONNESSO / ISOLATO (1, 2, 2) -> dà bonus di ventilazione (VENTILATED)
        BlockState isolatedWall = Blocks.COBBLESTONE_WALL.getDefaultState();
        context.setBlockState(wallPos, isolatedWall);
        ToxicAirEvent.MiasmaResult resultWallIsolated = ToxicAirEvent.calculateMiasma(context.getWorld(), context.getAbsolutePos(center));
        if (resultWallIsolated.ventilationType != ToxicAirEvent.RoomVentilationType.VENTILATED) {
            context.throwPositionedException("Il muretto isolato su parete laterale deve risultare VENTILATED, trovato: " + resultWallIsolated.ventilationType, center);
        }
        if (resultWallIsolated.ventilationScore <= 0.0) {
            context.throwPositionedException("Il muretto isolato su parete deve conferire un bonus di ventilazione > 0!", center);
        }
        context.setBlockState(wallPos, Blocks.STONE.getDefaultState());
        ...
        ToxicAirEvent.MiasmaResult resultWallCeiling = ToxicAirEvent.calculateMiasma(context.getWorld(), context.getAbsolutePos(center));
        if (resultWallCeiling.ventilationType != ToxicAirEvent.RoomVentilationType.VENTILATED) {
            context.throwPositionedException("Il muretto sul soffitto deve risultare VENTILATED, trovato: " + resultWallCeiling.ventilationType, center);
        }
        if (resultWallCeiling.ventilationScore <= 0.0) {
            context.throwPositionedException("Il muretto sul soffitto deve conferire un bonus di ventilazione > 0!", center);
        }
        context.setBlockState(ceilingPos, Blocks.STONE.getDefaultState());

        // 5. Muretto sul pavimento (2, 1, 2) verso esterno/aria sotto -> conferisce bonus VENTILATED
        BlockPos floorPos = new BlockPos(2, 1, 2);
        context.setBlockState(floorPos, Blocks.COBBLESTONE_WALL.getDefaultState());
        context.setBlockState(new BlockPos(2, 0, 2), Blocks.AIR.getDefaultState());
        context.setBlockState(new BlockPos(1, 0, 2), Blocks.AIR.getDefaultState());
        context.setBlockState(new BlockPos(0, 0, 2), Blocks.AIR.getDefaultState());
        ToxicAirEvent.MiasmaResult resultWallFloor = ToxicAirEvent.calculateMiasma(context.getWorld(), context.getAbsolutePos(center));
        if (resultWallFloor.ventilationType != ToxicAirEvent.RoomVentilationType.VENTILATED) {
            context.throwPositionedException("Il muretto sul pavimento verso l'esterno deve risultare VENTILATED, trovato: " + resultWallFloor.ventilationType, center);
        }
        if (resultWallFloor.ventilationScore <= 0.0) {
            context.throwPositionedException("Il muretto sul pavimento deve conferire un bonus di ventilazione > 0!", center);
        }
    ...
    public void testDecorativeBlocksDoNotObstruct(TestContext context) {
        ...
        context.setBlockState(new BlockPos(2, 2, 1), Blocks.CHAIN.getDefaultState().with(net.minecraft.block.ChainBlock.AXIS, Direction.Axis.Y));
        ...
        ToxicAirEvent.MiasmaResult result = ToxicAirEvent.calculateMiasma(context.getWorld(), context.getAbsolutePos(center));
        if (result.volume < 2) {
            context.throwPositionedException("Le catene/decorazioni non devono ostacolare la diffusione dell'aria (volume >= 2), trovato: " + result.volume, center);
        }
        if (result.toxicScore <= 0.0) {
    ...
    public void testIronBarsVentilation(TestContext context) {
        ...
        ToxicAirEvent.MiasmaResult result = ToxicAirEvent.calculateMiasma(context.getWorld(), context.getAbsolutePos(center));
        if (result.ventilationType != ToxicAirEvent.RoomVentilationType.VENTILATED) {
            context.throwPositionedException("Le grate di ferro verso l'esterno devono risultare VENTILATED, trovato: " + result.ventilationType, center);
        }
        if (result.ventilationScore <= 0.0) {
            context.throwPositionedException("Le grate di ferro devono conferire un bonus di ventilazione > 0!", center);
        }
    ...
    public void testFenceGateClosedIsVentilatedAndOpenIsClean(TestContext context) {
        ...
        // 1. Cancelletto Chiuso (OPEN = false) -> deve dare bonus VENTILATED (+3.0) come le staccionate
        ...
        if (resultClosed.ventilationType != ToxicAirEvent.RoomVentilationType.VENTILATED) {
            context.throwPositionedException("Il cancelletto chiuso deve risultare VENTILATED, trovato: " + resultClosed.ventilationType, center);
        }
        if (resultClosed.ventilationScore != 3.0) {
            context.throwPositionedException("Il cancelletto chiuso deve dare ventilationScore = 3.0, trovato: " + resultClosed.ventilationScore, center);
        }
        ...
        ToxicAirEvent.MiasmaResult resultOpen = ToxicAirEvent.calculateMiasma(context.getWorld(), context.getAbsolutePos(center));
        if (resultOpen.ventilationType != ToxicAirEvent.RoomVentilationType.CLEAN_OPEN_AIR) {
            context.throwPositionedException("Il cancelletto aperto verso l'esterno deve risultare CLEAN_OPEN_AIR, trovato: " + resultOpen.ventilationType, center);
        }
    ...
    public void testTrapdoorFloorAndCeilingOrientations(TestContext context) {
        ...
        ToxicAirEvent.MiasmaResult resultCeilingClosed = ToxicAirEvent.calculateMiasma(context.getWorld(), context.getAbsolutePos(center));
        if (resultCeilingClosed.ventilationType != ToxicAirEvent.RoomVentilationType.HERMETIC_SEALED) {
            context.throwPositionedException("Botola chiusa sul soffitto deve risultare HERMETIC_SEALED, trovato: " + resultCeilingClosed.ventilationType, center);
        }
        ...
        ToxicAirEvent.MiasmaResult resultCeilingOpen = ToxicAirEvent.calculateMiasma(context.getWorld(), context.getAbsolutePos(center));
        if (resultCeilingOpen.ventilationType != ToxicAirEvent.RoomVentilationType.CLEAN_OPEN_AIR) {
            context.throwPositionedException("Botola aperta sul soffitto deve risultare CLEAN_OPEN_AIR, trovato: " + resultCeilingOpen.ventilationType, center);
        }
        context.setBlockState(ceilingPos, Blocks.STONE.getDefaultState());
        ...
        ToxicAirEvent.MiasmaResult resultFloorClosed = ToxicAirEvent.calculateMiasma(context.getWorld(), context.getAbsolutePos(center));
        if (resultFloorClosed.ventilationType != ToxicAirEvent.RoomVentilationType.HERMETIC_SEALED) {
            context.throwPositionedException("Botola chiusa sul pavimento deve risultare HERMETIC_SEALED, trovato: " + resultFloorClosed.ventilationType, center);
        }
        ...
        ToxicAirEvent.MiasmaResult resultFloorOpen = ToxicAirEvent.calculateMiasma(context.getWorld(), context.getAbsolutePos(center));
        if (resultFloorOpen.ventilationType != ToxicAirEvent.RoomVentilationType.CLEAN_OPEN_AIR) {
            context.throwPositionedException("Botola aperta sul pavimento verso l'esterno deve risultare CLEAN_OPEN_AIR, trovato: " + resultFloorOpen.ventilationType, center);
        }
