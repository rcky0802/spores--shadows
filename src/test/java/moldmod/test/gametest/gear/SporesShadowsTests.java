...
public class SporesShadowsTests {
    ...
    public void testWaxingConsumesHoneycombAndSetsWaxed(TestContext context) {
        ...
        net.minecraft.entity.player.PlayerEntity player = context
                .createMockPlayer(net.minecraft.world.GameMode.SURVIVAL);
        ...
        if (honeycomb.getCount() != 4) {
            context.throwPositionedException("Honeycomb was not consumed! Expected 4, got " + honeycomb.getCount(),
                    pos);
        }
    ...
    public void testScrapingWaxTakesPriorityAndConsumesDurability(TestContext context) {
        ...
        BlockState waxedTaintedLog = ModBlocks.MOLDY_TO_WAXED.get(ModBlocks.VANILLA_TO_MOLDY.get(Blocks.OAK_LOG))
                .getDefaultState()
    ...
    public void testStripping(TestContext context) {
        ...
        BlockState cleanLog = ModBlocks.VANILLA_TO_MOLDY.get(Blocks.OAK_LOG).getDefaultState().with(MoldyLogBlock.STAGE,
                0);
        context.setBlockState(pos, cleanLog);
    ...
    public void testHumidityCalculation(TestContext context) {
        ...
        double localBonus = MoldRiskCalculator
                .calculate(context.getWorld(), context.getAbsolutePos(center), false, cleanLog).localHumidityBonus();
    ...
    public void testDropRottenNoSilkTouch(TestContext context) {
        ...
        net.minecraft.entity.player.PlayerEntity player = context
                .createMockPlayer(net.minecraft.world.GameMode.SURVIVAL);
        java.util.List<ItemStack> drops = net.minecraft.block.Block.getDroppedStacks(rottenLog, context.getWorld(),
                context.getAbsolutePos(new BlockPos(0, 0, 0)), null, player, player.getMainHandStack());

        if (!drops.isEmpty()) {
            context.throwPositionedException(
                    "Rotten log should drop nothing without Silk Touch! Dropped: " + drops.get(0).getItem(),
                    new BlockPos(0, 0, 0));
        }
    ...
    public void testDropRottenWithSilkTouch(TestContext context) {
        ...
        silkTouchPick.addEnchantment(
                context.getWorld().getRegistryManager().get(net.minecraft.registry.RegistryKeys.ENCHANTMENT)
                        .getEntry(net.minecraft.enchantment.Enchantments.SILK_TOUCH).get(),
                1);

        java.util.List<ItemStack> drops = net.minecraft.block.Block.getDroppedStacks(rottenLog, context.getWorld(),
                context.getAbsolutePos(new BlockPos(0, 0, 0)), null, player, silkTouchPick);

        if (drops.isEmpty() || !drops.get(0).getItem()
                .equals(net.minecraft.registry.Registries.ITEM.get(moldmod.SporesShadows.id("rotten_oak_log")))) {
            context.throwPositionedException("Rotten log should drop 'rotten_oak_log' with Silk Touch!",
                    new BlockPos(0, 0, 0));
        }
    ...
    public void testDropTainted(TestContext context) {
        ...
        net.minecraft.entity.player.PlayerEntity player = context
                .createMockPlayer(net.minecraft.world.GameMode.SURVIVAL);
        java.util.List<ItemStack> drops = net.minecraft.block.Block.getDroppedStacks(taintedLog, context.getWorld(),
                context.getAbsolutePos(new BlockPos(0, 0, 0)), null, player, player.getMainHandStack());

        if (drops.isEmpty() || !drops.get(0).getItem()
                .equals(net.minecraft.registry.Registries.ITEM.get(moldmod.SporesShadows.id("tainted_oak_log")))) {
            context.throwPositionedException("Tainted log should drop 'tainted_oak_log' even without Silk Touch!",
                    new BlockPos(0, 0, 0));
        }
    ...
    public void testPlacementConversion(TestContext context) {
        ...
        // Place on the EAST face of the block below, which should result in X axis for
        // PillarBlock
        ...
        net.minecraft.item.ItemPlacementContext placementContext = new net.minecraft.item.ItemPlacementContext(player,
                Hand.MAIN_HAND, oakLogItem, hit);

        net.minecraft.util.ActionResult result = ((net.minecraft.item.BlockItem) oakLogItem.getItem())
                .place(placementContext);
        ...
        if (placedState.contains(net.minecraft.state.property.Properties.AXIS)
                && placedState.get(net.minecraft.state.property.Properties.AXIS) != Direction.Axis.X) {
            context.throwPositionedException("Axis was not preserved! Expected X, got "
                    + placedState.get(net.minecraft.state.property.Properties.AXIS), new BlockPos(0, 1, 0));
        }
    ...
    private void simulatePlayerUse(TestContext context, BlockPos pos, net.minecraft.entity.player.PlayerEntity player) {
        net.minecraft.util.hit.BlockHitResult hit = new net.minecraft.util.hit.BlockHitResult(
                context.getAbsolutePos(pos).toCenterPos(), Direction.UP, context.getAbsolutePos(pos), false);
        net.minecraft.util.ActionResult result = net.fabricmc.fabric.api.event.player.UseBlockCallback.EVENT.invoker()
                .interact(player, context.getWorld(), Hand.MAIN_HAND, hit);
