...
public class JadeNoteBlockProviderGameTests {
    ...
    private static BlockAccessor createMockAccessor(net.minecraft.world.World world, BlockPos pos, BlockState state) {
    ...
    private static ITooltip createMockTooltip(List<Text> list) {
        return (ITooltip) java.lang.reflect.Proxy.newProxyInstance(
                ...
                (proxy, method, args) -> {
                    if ("add".equals(method.getName()) && args != null && args.length > 0 && args[0] instanceof Text text) {
                        list.add(text);
                    }
                    return null;
                }
    ...
    public void testNoteCalculations(TestContext context) {
        ...
        // Test octave colors
        context.assertTrue(MoldyNoteBlockProvider.getOctaveColor(0) == Formatting.AQUA, "Octave 0 must be AQUA");
        context.assertTrue(MoldyNoteBlockProvider.getOctaveColor(11) == Formatting.AQUA, "Octave 0 upper bound must be AQUA");
        context.assertTrue(MoldyNoteBlockProvider.getOctaveColor(12) == Formatting.GREEN, "Octave 1 lower bound must be GREEN");
        context.assertTrue(MoldyNoteBlockProvider.getOctaveColor(23) == Formatting.GREEN, "Octave 1 upper bound must be GREEN");
        context.assertTrue(MoldyNoteBlockProvider.getOctaveColor(24) == Formatting.YELLOW, "Octave 2 must be YELLOW");
    ...
    public void testNoteBlockTooltipOutputAcrossStages(TestContext context) {
        ...
        MoldyNoteBlockProvider.INSTANCE.appendTooltip(createMockTooltip(lines0),
                createMockAccessor(context.getWorld(), pos, moldyNoteBlock.with(MoldyBlock.STAGE, 0).with(MoldyBlock.WAXED, false)), null);
        context.assertTrue(lines0.isEmpty(), "Stage 0 should not produce custom line (handled by vanilla Jade)");
        ...
        MoldyNoteBlockProvider.INSTANCE.appendTooltip(createMockTooltip(lines1),
                createMockAccessor(context.getWorld(), pos, moldyNoteBlock.with(MoldyBlock.STAGE, 1).with(MoldyBlock.WAXED, false)), null);
        ...
        MoldyNoteBlockProvider.INSTANCE.appendTooltip(createMockTooltip(lines2),
                createMockAccessor(context.getWorld(), pos, moldyNoteBlock.with(MoldyBlock.STAGE, 2).with(MoldyBlock.WAXED, false)), null);
        ...
        MoldyNoteBlockProvider.INSTANCE.appendTooltip(createMockTooltip(lines3),
                createMockAccessor(context.getWorld(), pos, moldyNoteBlock.with(MoldyBlock.STAGE, 3).with(MoldyBlock.WAXED, false)), null);
        ...
        MoldyNoteBlockProvider.INSTANCE.appendTooltip(createMockTooltip(linesWaxed),
                createMockAccessor(context.getWorld(), pos, moldyNoteBlock.with(MoldyBlock.STAGE, 2).with(MoldyBlock.WAXED, true)), null);
